...
class N_pair_loss(torch.nn.Module):
    ...
    def forward(self, vfeat, afeat, u=0.1, margin=1):
        ...
        loss_reg = (u / (2 * bn)) * (
            torch.sum(torch.norm(vfeat, p=2, dim=1)) + torch.sum(torch.norm(afeat, p=2, dim=1)))

        return loss_ls + loss_reg
