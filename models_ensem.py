...
class lstm_loss(nn.Module):
    ...
    def forward(self, sim, target, margin=1):
        ...
        loss_balance1 = F.relu(
            0.9 - torch.pow(torch.mean(torch.pow(sim_0[0:bs / 2], 1)) - torch.mean(torch.pow(sim_0[bs / 2:bs], 1)),2))
        loss_balance2 = F.relu(
            0.9 - torch.pow((torch.mean(torch.pow(sim_1[bs / 2:bs], 1)) - torch.mean(torch.pow(sim_1[0:bs / 2], 1))),2))
